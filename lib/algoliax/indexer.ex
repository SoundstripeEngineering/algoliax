defmodule Algoliax.Indexer do
  ...
  ## Example
      ...
      iex> People.search_multiple([%{indexName: "index_1", params: %{query: "jon"}}, %{indexName: "index_2", params: %{query: "bar"}}])
      {:ok,
        %{
          response: %{
            "results" => [
  ...
  """
  @callback search_multiple(queries :: list(map()), strategy :: String.t()) ::
              {:ok, list(Algoliax.Response.t())} | {:not_indexable, model :: map()}
  ...
  defmacro __using__(settings) do
    quote do
      ...
      def search_multiple(queries, strategy \\ "none") do
        Search.search_multiple(__MODULE__, @settings, queries, strategy)
      end
